...
class Companion:
    ...
    def __init__(self, interface, save_result=False, print_debug=False):
        ...
        self.sessions_dir = f'{user_home}/.HuyPc/sessions/'
        self.pixiewps_dir = f'{user_home}/.HuyPc/pixiewps/'
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        if self.vuln_list:
            print('\n\nNetwork marks: {1} {0} {2} {0} {3}'.format(
                '|',
                colored('😋RIP xD', color='green'),
                colored('👿WPS Locked', color='red'),
                colored('🥱 Hacked', color='yellow')
            ))
        print('\n🔥Networks list:\n')
        print('{:<4} {:<17} {:<18} {:<18}'.format(
            '#', 'SSID', 'Security', 'Signal.', 'PWR', 'WSC device name', 'WSC model'))
